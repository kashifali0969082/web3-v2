...
export default function Downlines() {
  ...
  return (
    <div className="downlines">
      ...
      <div className="researchSection" style={{ marginTop:'0px', display: "flex", justifyContent: "center", alignItems: "center" }}>
        <input
        style={{width:'100%'}}
          value={query}
      ...
      <div className="customGridContainer">
        {filteredDownlines.length > 0 ? (
          filteredDownlines.map((downline) => (
            <CustomGridContainer key={downline.address} downline={downline} getRandomColor={getRandomColor} />
          ))
        ) : (
          <p style={{ textAlign: "center", color: "#aaa" }}>No matching downlines found.</p>
        )}
      </div>
    </div>
  );
...
function CustomGridContainer({
  ...
  return (
    <div className="card" style={{  width: "100%", maxWidth: "400px", height: "auto", padding: "1rem"}}>
      <div style={{ backgroundColor: getRandomColor(), padding: "1rem" }} className="cardHeader">
      ...
      <div className="cardBody" style={{ overflow: "hidden" }}>
        <p><strong>Address:</strong> {downline.address}</p>
        <p><strong>Level:</strong> {downline.vrData?.currentUserLevel}</p>
        <p><strong>Total Income:</strong> {downline.vrData?.totalIncome.toFixed(2) || "0"} Sonic</p>
        <p><strong>Directs:</strong> {downline.downlines}</p>
      </div>
    </div>
  );
}
