...
export const StatBadge = ({ type, title, value, subtitle, icon }: StatBadgeProps) => {
    ...
    switch (type) {
      case 'earnings':
        return {
          ...
          badge: 'bg-emerald-900/60 text-emerald-400 border-emerald-700/50 backdrop-blur-sm',
          ...
          highlight: 'bg-gradient-to-r from-green-600 via-emerald-400 to-green-600',
      ...
      case 'network':
        return {
          ...
          badge: 'bg-blue-900/60 text-blue-400 border-blue-700/50 backdrop-blur-sm',
          text: 'text-blue-400',
      ...
      case 'premium':
        return {
          ...
          badge: 'bg-amber-900/60 text-amber-400 border-amber-700/50 backdrop-blur-sm',
          text: 'text-amber-400',
          lightText: 'text-amber-300/90',
          valueText: 'font-mono tracking-tight',
          icon: 'bg-amber-900/40 text-amber-400 shadow-inner shadow-amber-900/40',
          highlight: 'bg-gradient-to-r from-amber-600 via-amber-400 to-amber-600',
  ...
  return (
    <Card className={`${styles.card} border overflow-hidden relative ${styles.glow}`}>
      <CardContent className="p-0 relative">
        ...
        <div className="p-6 relative">
          ...
          <div className="flex justify-between items-center mb-5">
            <div className={`p-2.5 rounded-full ${styles.icon} backdrop-blur-sm`}>
              {icon || <BadgeIcon type={type} />}
            </div>
            <div className={`px-3 py-1 text-xs font-medium rounded-full ${styles.badge} border flex items-center`}>
              {type === 'premium' && <span className="mr-1 text-yellow-300">★</span>}
              {subtitle}
            </div>
          </div>
          ...
          {/* Value and title */}
          <div>
            <div className={`text-3xl font-extrabold mb-1 ${styles.valueText} flex items-baseline relative`}>
              ...
              {(type === 'earnings' || type === 'network') && 
                <span className="ml-1 text-lg opacity-90 font-bold text-white/90">S</span>
              }
              
              {/* Subtle glowing effect under the number */}
              <div className="absolute -bottom-1 left-0 w-full h-[2px] opacity-40 rounded-full" 
                style={{ 
                  background: type === 'earnings' 
            ...
            {(type === 'earnings' || type === 'network') && typeof value === 'number' && (
              <div className="text-sm font-medium text-white/60 mb-1">
                {/* {getUsdValue(value).usdFormatted} USD */}
              </div>
            )}
            <div className={`text-sm font-medium ${styles.lightText}`}>{title}</div>
          </div>
